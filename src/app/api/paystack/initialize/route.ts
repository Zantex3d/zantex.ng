...
export async function POST(req: Request) {
  try {
    ...
    let amount = 0;
    if (hostel === 'girls' || hostel === 'unimaid-girls' || hostel === 'medical-nursing' || hostel === 'ard-hblocks') {
      switch (plan) {
...
export async function POST(req: Request) {
  try {
    ...
    const hostel = rawHostel === 'unimaid-girls'
      ? 'girls'
      : rawHostel;

    // Determine amount (same pricing for all hostels)
    let amount = 0;
